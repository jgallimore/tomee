...
public class AbstractJAXRSFactoryBean extends AbstractEndpointFactory {
        ...
        if (server) {
            for (Iterator<ClassResourceInfo> it = list.iterator(); it.hasNext();) {
                ...
                if (!isValidClassResourceInfo(cri)) {
                    LOG.info("Not using " + cri.getServiceClass().getName() + " as it is not valid");
                    it.remove();
                }
            }
        }
    ...
    protected boolean isValidClassResourceInfo(ClassResourceInfo cri) {
        ...
        final boolean valid = !(cri.isCreatedFromModel() && serviceCls == cri.getResourceClass()
                && !InjectionUtils.isConcreteClass(serviceCls));

        LOG.info("Checking: " + serviceCls.getName() + "\n\tCreated from model: " + cri.isCreatedFromModel() + "\n\t" +
                "Service class = resource class: " + Boolean.toString(serviceCls == cri.getResourceClass()) + "\n\t" +
                "Is concrete class: " + InjectionUtils.isConcreteClass(serviceCls) + "\n\t" +
                "Valid: " + valid);
